...
static void g_yang_doTask(int32_t taskId, void *user) {
    ...
    if (!session->isControlled && session->isSendStun && taskId == 1) {
        if (session->context.stun.data && session->context.stun.nb > 0) {
            if (session->context.sock->write(&session->context.sock->session, session->context.stun.data,
                                             session->context.stun.nb) != Yang_Ok) {
    ...
    if (session->startRecv && session->play) {
        if (taskId == 1) {
            if (session->play->send_rtcp_rr(&session->context, session->play->playStream))
    ...
    if (session->push) {
        if (taskId == 1) {
            if (session->push->send_rtcp_sr(&session->context, session->push->pubStream))
                yang_error("send rtcp sr Error ");
            if (session->push->check_bandwidth(&session->context, session->push->pubStream))
                yang_error("check bandwidth Error ");
            if (session->push->check_twcc(&session->context, session->push->pubStream))
...
static int32_t yang_rtcconn_dispatch_rtcp(YangRtcSession *session, YangRtcpCommon *rtcp) {
    ...
    if (session->play && Yang_Ok != (err = session->play->
                                     on_rtcp(&session->context, session->play->playStream, rtcp))) {
...
#if Yang_Enable_Dtls
    if (session == NULL || p == NULL || session->context.state != Yang_Conn_State_Connected || session->context.dtls->
        session.state != YangDtlsStateClientDone)
        return Yang_Ok;
#else
	if (session==NULL||p==NULL||session->context.state!=Yang_Conn_State_Connected)	return Yang_Ok;
#endif
...
#else
        if (session->context.dtls->processData(NULL, &session->context.dtls->session, data, size) == Yang_Ok && session
            ->context.state == Yang_Conn_State_Connecting) {
#endif
...
client_sucess:
    if (session->context.state == Yang_Conn_State_Connecting) {
        ...
        if (session->context.streamConfig && session->context.streamConfig->rtcCallback.sendRequest)
            session->context.streamConfig->rtcCallback.sendRequest(session->context.streamConfig->rtcCallback.context,
...
static void yang_rtcconn_on_ice(YangRtcSession *session, char *remoteIp, int32_t port) {
    ...
    YangRtcSocket *sockSession = session->context.sock;
    yang_printf("updateRemoteAddress:%s,%d", remoteIp, port);
    sockSession->updateRemoteAddress(&sockSession->session, remoteIp, port);
}

static void yang_rtcconn_addRemoteCandidate(YangRtcSession *session, const char *candidate) {
    if (!session || !candidate) {
        yang_error("[RTC] Failed to add remote candidate: invalid session, ICE session, or candidate");
        return;
    }

    // Variables pour stocker les informations du candidat ICE
    char foundation[32];
    ...
    // Analyser le candidat ICE (format SDP)
    int parsed = sscanf(
        candidate, "candidate:%31s %u %7s %u %63s %hu typ %15s generation %u ufrag %31s network-id %u network-cost %u",
        foundation, &componentId, transport, &priority, ip, &port, type, &generation, ufrag, &networkId, &networkCost);

    // Vérifier que tous les champs ont été correctement analysés
    if (parsed != 11) {
        yang_error("[RTC] Failed to parse ICE candidate: invalid format");
        return;
    }

    // Afficher les informations du candidat pour le débogage
    yang_trace(
        "[RTC] Adding remote ICE candidate: foundation=%s, componentId=%u, transport=%s, priority=%u, ip=%s, port=%hu, type=%s, generation=%u, ufrag=%s, network-id=%u, network-cost=%u",
        foundation, componentId, transport, priority, ip, port, type, generation, ufrag, networkId, networkCost);

    yang_rtcconn_on_ice(session, ip, port);
}
...
int32_t yang_rtcconn_getRemoteSdp(YangRtcSession *session, char *sdpstr) {
    ...
    if (session->remote_audio == NULL)
        session->remote_audio = (YangAudioParam *)
                yang_calloc(sizeof(YangAudioParam), 1);
#endif
#if Yang_Enable_RTC_Video
...
static int32_t yang_rtcconn_createOffer(YangRtcSession *session, char **psdp) {
    ...
    int32_t err = Yang_Ok;
    if (!session->context.avinfo->rtc.iceUsingLocalIp && (err = session->ice.initIce(&session->ice.session)) !=
        Yang_Ok) {
        return yang_error_wrap(err, "ice request fail(%s)",
...
static int32_t yang_rtcconn_setLocalDescription(YangRtcSession *session, char *sdp) {
    ...
    int32_t err = Yang_Ok;
        yang_printf("Set local description");
    ...
    if (session->context.avinfo->sys.mediaServer == Yang_Server_P2p && !session->context.avinfo->rtc.iceUsingLocalIp) {
        ...
        if (session->ice.session.candidateType == YangIceTurn && session->ice.session.iceState == YangIceSuccess)
            yang_printf("Returning yang ok");

            return Yang_Ok;
    }

        yang_printf("Creating rtc socket success");
    err = yang_create_rtcsocket(session->context.sock, session->context.avinfo->sys.familyType,
...
static int32_t yang_rtcconn_setRemoteDescription(YangRtcSession *session, char *sdpstr) {
    ...
    int32_t err = Yang_Ok;
        ...
        yang_printf("Setting remote desc");
    if (session->context.avinfo->sys.mediaServer == Yang_Server_P2p && !session->context.avinfo->rtc.iceUsingLocalIp
    ...
    if (session->context.avinfo->sys.mediaServer == Yang_Server_P2p && !session->context.avinfo->rtc.iceUsingLocalIp) {
        ...
        if ((err = session->ice.iceHandle(&session->ice.session, session, yang_rtcconn_turn_receive,
                                          session->context.streamConfig->remoteIp,
                                          session->context.streamConfig->remotePort)) != Yang_Ok) {
            return yang_error_wrap(err, "setLocalDescription error!");
        }


        yang_printf("Creating socket");
        if (session->ice.session.candidateType == YangIceTurn && session->ice.session.iceState == YangIceSuccess) {
            yang_printf("Im in if");
            err = yang_create_rtcsocket(session->context.sock, session->context.avinfo->sys.familyType,
                                        Yang_Socket_Protocol_Udp, session->context.streamConfig->localPort);
            if (err != Yang_Ok) return yang_error_wrap(err, "setLocalDescription error!");
    ...
    if (!session->isControlled)
        session->context.sock->updateRemoteAddress(&session->context.sock->session,
                                                   session->context.streamConfig->remoteIp,
...
int32_t yang_create_rtcConnection(YangRtcConnection *conn, YangStreamConfig *streamconfig, YangAVInfo *avinfo) {
    ...
    conn->addRemoteCandidate = yang_rtcconn_addRemoteCandidate;
