...
void yang_getH265RtmpHeader(uint8_t *meta, uint8_t *src, int32_t *hLen) {
				...
				yang_memcpy(src,s,4);
				yang_memcpy(src+4,meta+vpsPos,vpsLen);
				pos+=4+vpsLen;
				yang_memcpy(src+pos,s,4);
				pos+=4;
				yang_memcpy(src+pos,meta+spsPos,spsLen);
				pos+=spsLen;
				yang_memcpy(src+pos,s,4);
				pos+=4;
				yang_memcpy(src+pos,meta+ppsPos,ppsLen);
				pos+=ppsLen;
				*hLen=pos;
...
void yang_getH264RtmpHeader(uint8_t *buf, uint8_t *src, int32_t *hLen) {
	...
	*(src + 3) = 0x01;
	yang_memcpy(src + 4, spsStart, spsLen);
	*(src + 4 + spsLen + 3) = 0x01;
	yang_memcpy(src + 4 + spsLen + 4, ppsStart, ppsLen);
