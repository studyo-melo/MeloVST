...
void YangVideoCaptureHandle::putBufferNv21(int64_t startTime, uint8_t *data_y, uint8_t* data_u){
...
void YangVideoCaptureHandle::putBufferAndroid2(int64_t startTime, uint8_t *data_y, uint8_t* data_u,uint8_t* data_v){
	if(m_androidBuf==NULL){
		m_androidBuf=new uint8_t[m_bufLen];
	}

	if(data_y) memcpy(m_androidBuf,data_y,m_ylen);
	if(data_u) memcpy(m_androidBuf+m_ylen,data_u,m_ulen);
	if(data_v) memcpy(m_androidBuf+m_ylen+m_ulen,data_v,m_ulen);
