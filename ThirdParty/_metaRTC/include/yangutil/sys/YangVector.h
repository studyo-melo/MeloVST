...
void yang_insert_##x##Vector(x##Vector* vec,x* value){ \
	...
	if(vec->vsize>=vec->capacity){\
		x* tmp=(x*)yang_calloc(sizeof(x)*(vec->capacity+5),1);\
		yang_memcpy(tmp,vec->payload,sizeof(x)*vec->vsize);\
		yang_free(vec->payload);\
		vec->payload=tmp;\
		vec->capacity+=5;\
	}\
	if(value)\
		yang_memcpy(&vec->payload[vec->vsize++],value,sizeof(x));\
	else\
		yang_memset(&vec->payload[vec->vsize++],0,sizeof(x));\
}\
void yang_create_##x##Vector(x##Vector* vec){\
	vec->capacity=5;\
	vec->payload=(x*)yang_calloc(vec->capacity*sizeof(x),1);\
	vec->vsize=0;\
}\
void yang_destroy_##x##Vector(x##Vector* vec){\
	vec->vsize=0;\
...
void yang_clear_##x##Vector(x##Vector* vec){\
	yang_memset(vec->payload,0,vec->capacity*sizeof(x));\
	vec->vsize=0;\
}\
void yang_remove_##x##Vector(x##Vector* vec,int32_t index){\
	if(vec==NULL||vec->vsize==0||index>=vec->vsize) return;\
	if(vec->vsize==1) {yang_clear_##x##Vector(vec);return;}\
	if(vec->vsize!=index+1){yang_memmove((char*)vec->payload+index*sizeof(x),(char*)vec->payload+(index+1)*sizeof(x),sizeof(x)*(vec->vsize-index-1));}\
	vec->vsize--;\
}\
...
void yang_create_##x##Vector2(x##Vector2* vec){\
	vec->vec.vsize=0;\
	vec->vec.capacity=5;\
	vec->vec.payload=(x**)yang_calloc(vec->vec.capacity*sizeof(x*),1);\
